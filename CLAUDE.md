...
The frontend runs from `frontend/`: `npm run dev` (Vite, port 5173), `npm run build` (`tsc -b && vite build`).
...
- `frontend/src/lib/axios.ts` — one axios instance; a bearer-token interceptor attaches the JWT, and any `401` clears the token and bounces to `/login`.
